...
import type { SkuInfo, SkuListResponseData, AllSkuListResponseData, SkuInfoResponseData } from "@/api/product/type";
...
export const reqAllSkuList = (page: number, limit: number) => {
  return request.get<any, AllSkuListResponseData>(API.SKU_LIST_URL + `${page}/${limit}`)
}
