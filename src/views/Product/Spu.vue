      ...
      <el-table border style="width: 100%; margin: 10px 0" :data="spuRecords">
        ...
        <el-table-column label="操作" width="240">
          ...
          <template #="{ row }">
            ...
            <el-button type="warning" size="small" icon="Edit" title="编辑SPU" @click="spuEdit(row)" ></el-button>
            <el-button type="info" size="small" icon="View" title="查看SKU列表" @click="findSku(row)"></el-button>
            <!-- 气泡确认是否删除 -->
            <el-popconfirm title="确定要删除?" width="200" @confirm="deleteSpuHandler(row.id)">
              <template #reference>
...
const addSku = (row: any) => {
  ...
  scene.value = 2
  // 初始化sku数据
  skuRef.value.initSkuData(categoryStore.category1Id, categoryStore.category2Id, row)
...
const getSpuList = async () => {
  const { code, data } = await reqSpuList(currentPage.value, pageSize.value, categoryStore.category3Id as number)
