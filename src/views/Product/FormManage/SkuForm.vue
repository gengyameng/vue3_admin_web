  ...
  <!-- SKU添加 -->
   <el-form label-width="100px">
    <el-form-item label="SKU名称">
      <el-input placeholder="SKU名称" v-model="skuInfoParams.skuName"></el-input>
    </el-form-item>
    <el-form-item label="价格(元)">
      <el-input placeholder="价格(元)" v-model.number="skuInfoParams.price"></el-input>
    </el-form-item>
    <el-form-item label="重量(克)">
      <el-input placeholder="重量(克)" v-model="skuInfoParams.weight"></el-input>
    </el-form-item>
    <el-form-item label="SKU描述">
      <el-input type="textarea" placeholder="SKU描述" v-model="skuInfoParams.skuDesc"></el-input>
    </el-form-item>
    <el-form-item label="平台属性">
      <!-- 多个属性 -->
       <el-form :inline="true"  label-width="50px">
        <el-form-item
          v-for="item in attrArr" :key="item.id"
          :label="item.attrName"
        ...
        <el-table-column label="操作">
          <!-- 添加按钮 -->
           <template #="{row}">
            ...
            >设置默认</el-button>
            <el-button v-else
              type="success" disabled>默认图片</el-button>
           </template>
        </el-table-column>
      </el-table>
    </el-form-item>
    <el-form-item>
      <el-button type="primary" size="default" @click="saveSkuhandler">保存</el-button>
...
import type { SkuInfo, SpuInfo, AttrInfo, SpuSaleAttr, SpuImage } from '@/api/product/type'
...
// 提供初始化方法对外暴露
const initSkuData = async (category1Id: number, category2Id: number, spu: SpuInfo) => {
  // 清空数据
  ...
  // 1. 获取平台属性
  const attrResult = await reqAttrList(category1Id, category2Id, spu.category3Id)
  // 2. 获取当前spu的销售属性
...
const saveSkuhandler = async() => {
  // 整理参数
  // 平台数据
  skuInfoParams.skuAttrValueList = attrArr.value.reduce((prev: any, next: any) => {
  ...
  // 销售属性
  skuInfoParams.skuSaleAttrValueList = saleArr.value.reduce((prev:any, next: any) => {
