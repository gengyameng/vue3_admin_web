     ...
     <el-table border style="margin-bottom: 15px;" :data="skuRecords">
      ...
      <el-table-column label="操作" width="250px" fixed="right">
        <template #="{row}">
          ...
          <!-- 展示 -->
          <el-button type="info" size="small" icon="View" @click="viewSkuHandler(row.id)"></el-button>
          <!-- 删除 -->
          <el-popconfirm title="确定要删除吗?" @confirm="deleteSkuHandler(row.id)">
            <template #reference>
        ...
        <el-row style="margin: 10px 0px;">
          ...
          <el-col :span="18">
            <el-tag 
              ...
              type="danger"
              v-for="attr in skuInfo.skuAttrValueList" :key="attr.id">{{ attr.valueName }}</el-tag>
          </el-col>
        </el-row>
        <el-row style="margin: 10px 0px;">
          ...
          <el-col :span="18">
            <el-tag 
              style="margin: 2px 5px;" 
              type="success"
              v-for="sale in skuInfo.skuSaleAttrValueList" :key="sale.id">{{ sale.saleAttrValueName }}</el-tag>
          </el-col>
        </el-row>
        <el-row style="margin: 10px 0px;">
...
import { reqAllSkuList, reqSaleSku, reqCancelSku, reqDeleteSku, reqFindSku } from '@/api/product/sku'
...
const getSkuList = async() => {
  const { code, message, data, ok } = await reqAllSkuList(currentPage.value, pageSize.value)
  if (code === 200) {
    console.log(data);
