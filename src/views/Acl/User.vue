<template>
  ...
  <el-card style="height: 70px;">
    <el-form :inline="true" class="search-form">
      ...
      <el-form-item>
        <el-button type="primary" :disabled="keyword ? false : true" @click="searchHandler">搜索</el-button>
  ...
  <el-card style="margin-top: 10px;">
    ...
    <el-popconfirm title="确认要删除吗?" @confirm="deleteSelectUserHandler">
      <template #reference>
        <el-button type="danger" :disabled="selectUserArr.length ? false : true">批量删除</el-button>
      </template>
    </el-popconfirm>
    ...
    >
      ...
      <el-table-column label="用户角色" show-overflow-tooltip prop="roleName"></el-table-column>
      ...
      <el-table-column label="操作" width="270px">
        <template #="{ row }">
          <!-- 三个按钮：分配角色、编辑、删除 -->
          <el-button type="success" icon="User" size="small" @click="assignUser(row)">分配角色</el-button>
          <el-button type="primary" icon="Edit" size="small" @click="updateUser(row)">编辑</el-button>
          <el-popconfirm :title="`确定要删除${row.username}吗?`" @confirm="deleteUser(row.id)">
            <template #reference>
     ...
     <template #default>
      ...
      >
        <el-form-item label="用户姓名" prop="username">
          <el-input v-model.trim="userForm.username" placeholder="请填写用户名字"></el-input>
        </el-form-item>
        <el-form-item label="用户昵称" prop="name">
          <el-input v-model.trim="userForm.name"placeholder="请填写用户昵称"></el-input>
        </el-form-item>
        <el-form-item label="用户密码" prop="password">
          <el-input v-model.trim="userForm.password" placeholder="请填写用户密码"></el-input>
        </el-form-item>
      </el-form>
    </template>
     <!-- 底部 -->
...
import { 
  reqUserList, reqAddOrUpdateUser, 
  reqUserRemove, reqUserBatchRemove,
  reqUserAssignInfo, reqAssignRole
} from '@/api/acl/user'
import type { 
  UserData, SaveUserFormData, UserInfo, UserRoleData, AssignRoleFormData
} from '@/api/acl/type'
...
const userRules = reactive<FormRules<typeof userForm>>({
  ...
  name: [{required: true, trigger: 'change', min:5, max: 10, message: '昵称在5-10位之间'}],
  password:[{required: true, min: 6, max: 16, message: "密码在6-16位之间", trigger: "change",}]
})
...
const getUserList = async () => {
  ...
  const { code, data } = await reqUserList(currentPage.value, pageSize.value, keyword.value)
    if (code === 200) {
