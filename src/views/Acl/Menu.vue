  ...
  <el-table border style="width: 100%" row-key="id" :data="menuArr">
    ...
    <el-table-column label="操作">
      <template #="{row}">
        ...
        >编辑</el-button>
        <el-popconfirm :title="`你确定要删除${row.name}?`" width="260px" @confirm="deleteMenuHandler(row.id)">
            <template #reference>
                <el-button
                  type="primary"
                  size="small"
    ...
    <template #default>
      ...
      >
        <el-form-item label="名称" prop="name">
          <el-input v-model.trim="menuParams.name" placeholder="请输入菜单名称"></el-input>
        </el-form-item>
        <el-form-item label="权限值" prop="code">
          <el-input v-model.trim="menuParams.code" placeholder="请输入权限值"></el-input>
        </el-form-item>
      </el-form>
    </template>
    <template #footer>
      <div class="dialog-footer">
        ...
        <el-button type="primary" @click="saveMenuHandler">
          确认
        </el-button>
      </div>
    </template>
...
import type {
  MenuPermissionData, setMenuForm
} from '@/api/acl/type'
...
const rules = {
  ...
  code: [
    { required: true, message: '权限值不可为空', trigger: 'blur' }
  ]
}
