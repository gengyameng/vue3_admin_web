<template>
  ...
  <el-card style="height: 70px;">
    <el-form :inline="true" class="search-form">
      ...
      <el-form-item>
        <!-- 两个按钮 -->
        <el-button :disabled="keyword ? false : true" type="primary"  @click="searchHandler">搜索</el-button>
  ...
  <el-card style="margin-top: 10px">
    ...
    <el-table border style="margin: 10px 0px" :data="roleRecords">
      ...
      <el-table-column label="操作" width="280px" align="center">
        <!-- 三个按钮：分配权限、编辑、删除 -->
        <template #="{ row }">
          <el-button type="primary" size="small" icon="User" @click="assignPermissionHandler(row.id)">分配权限</el-button>
          <el-button type="primary" size="small" icon="Edit" @click="editRole(row)">编辑</el-button>
          <el-popconfirm title="确认要删除吗?" width="200px" @confirm="deleteRoleHandler(row.id)">
            <template #reference>
...
import {
  reqRoleList, reqAddorUpdateRole, reqRemoveRole
} from '@/api/acl/role'
import { reqAllMenuList, reqSetRolePermission } from '@/api/acl/menu'
import type {
  RoleData, SaveRoleFormData, MenuPermissionData, SetRolePermissionForm
} from '@/api/acl/type'
